...
위성 이미지에서 건물 영역을 픽셀 단위로 분할하고, 예측 mask를 RLE 제출 형식으로 변환하는 AI/데이터 포트폴리오 프로젝트입니다.

이 저장소는 데이터 이해, baseline 구축, UNet++ 기반 개선, validation 설계, threshold sweep, TTA, 후처리, 제출 파일 생성까지 이어지는 end-to-end segmentation pipeline을 정리합니다.
...
주의: 위 수치는 공식 Private Leaderboard 점수가 아니라 내부 validation 결과입니다.
...
공식 대회 데이터와 SpaceNet 원본 데이터는 저장소에 포함하지 않습니다.
...
`data/`, `runs/`, model weights는 `.gitignore`로 제외되어 있습니다.
...
Detectron2 보조 실험은 별도 설치가 필요할 수 있습니다.
...
VS Code 실행과 디버깅 방법은 `VSCODE_RUN_DEBUG.md`에 정리되어 있습니다.
...
이 프로젝트는 최종 점수만 강조하지 않고, 아래 역량이 드러나도록 정리했습니다.
...
코드는 개인 포트폴리오와 학습 목적의 예시입니다. 공식 대회 데이터와 외부 데이터셋은 각 제공처의 이용 규정을 따르며, 이 저장소에는 포함하지 않습니다.
