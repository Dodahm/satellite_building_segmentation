...
이 폴더는 위성 이미지 건물 영역 분할을 빠르게 시작하기 위한 최소 베이스라인임.
...
중요:
- 원본 이미지와 CSV는 저장소 미포함.
- 이 코드는 실험 시작용 기본 구조이며, 최종 최적화 버전은 아님.
...
- 학습 이미지는 지정한 `image_size`로 resize.
- 손실 함수는 `BCEWithLogitsLoss + soft Dice loss`.
- 검증 점수는 threshold 적용 후 `hard Dice`로 저장.

## 4. EDA

`train` 중심 데이터 점검 스크립트 포함.
...
학습 후 최고 성능 checkpoint로 inference CSV 생성.
...
1. baseline 학습 완료.
2. `history.csv`에서 `val_dice` 변화 확인.
