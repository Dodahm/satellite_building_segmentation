...
# -----------------------------------------------------------------------------
# 이 스크립트는 원본 위성 이미지 데이터가 어디에 다운로드되었는지 점검하고,
# 표준 경로(`data/satellite_raw`)로 연결하기 위한 준비 스크립트다.
#
# 왜 필요한가:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-dir",
        type=Path,
        default=ROOT / "data/satellite_raw",
    )
    parser.add_argument(
        "--project-data-link",
        type=Path,
        default=ROOT / "projects/segmentation_baseline/data/satellite_raw",
    )
...
def looks_like_dataset_root(path: Path) -> tuple[bool, list[str]]:
    # 이 함수는 폴더가 원본 데이터 구조를 만족하는지 검사한다.
    required = [
        "train_img",
        "test_img",
        "train.csv",
...
def find_candidates() -> list[Path]:
    ...
    #
    # 변경 후:
    # - 주요 폴더를 순회하면서 원본 데이터 구조와 비슷한 경로를 찾는다.
    matches: list[Path] = []
    for root in candidate_roots():
        if not root.exists():
            continue
        for path in [root] + [p for p in root.iterdir() if p.is_dir()]:
            ok, _ = looks_like_dataset_root(path)
            if ok:
                matches.append(path)
    return matches
...
def main() -> None:
    ...
    if source_dir is not None:
        ok, missing = looks_like_dataset_root(source_dir)
        if not ok:
            print(f"[ERROR] source-dir 구조 불완전: {source_dir}")
        ...
        if not matches:
            print("[INFO] 원본 데이터 구조를 가진 폴더를 찾지 못함.")
    ...
    # 변경 전:
    # - 프로젝트 폴더 안쪽의 data/satellite_raw를 직접 채워야 했다.
    #
    # 변경 후:
    # - 원본 데이터는 표준 경로 하나에만 두고,
    #   프로젝트 내부 경로는 symlink로 연결한다.
    if source_dir.resolve() != args.target_dir.resolve():
        print("[WARN] 실제 파일 복사는 자동 수행하지 않음.")
        print("[WARN] 원본 데이터를 표준 경로로 이동하거나, 아래 명령으로 직접 복사:")
        print(f"       rsync -av '{source_dir}/' '{args.target_dir}/'")
    else:
        print("[OK] 원본 데이터가 이미 표준 경로에 있음.")
